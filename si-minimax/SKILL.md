---
name: smart-illustrator-minimax
description: smart-illustrator 的 minimax 替代版 — 文章配图与封面(用 minimax API 替代 Gemini,严格复用作者原生 mermaid/excalidraw 脚本,零侵入作者文件)。触发词:配图、插图、封面图、minimax、thumbnail、cover。
---
...
| `--cover-ratio <X:Y>` | `16:9` | 封面宽高比,如 `--cover-ratio 3:4`。minimax 实测仅确认支持 16:9;传其他比例时先 `python3 ~/.claude/skills/smart-illustrator/scripts/minimax_t2i.py -h` 实测支持列表,不支持 → 警告并回退 16:9 |
...
7. **唯一新依赖**: `~/.claude/skills/smart-illustrator/scripts/minimax_t2i.py`(随本仓库分发的 minimax 出图脚本,仅替代 `generate-image.ts` 里的 Gemini API 调用)
...
```bash
# 实测契约(2026-08-01):python3 ~/.claude/skills/smart-illustrator/scripts/minimax_t2i.py -h
...
[[ -n "$MINIMAX_IMAGE_API_KEY" || -n "$MINIMAX_API_KEY" ]] || {
  ...
  exit 1; }
[[ -f ~/.claude/skills/smart-illustrator/scripts/minimax_t2i.py ]] || { echo "[ERROR] ~/.claude/skills/smart-illustrator/scripts/minimax_t2i.py 不存在"; exit 1; }

python3 ~/.claude/skills/smart-illustrator/scripts/minimax_t2i.py "$(cat /tmp/si-prompt-${NN}.txt)" \
...
4. 比例:**显式初始化** `COVER_RATIO="16:9"; [[ -n "$COVER_RATIO_ARG" ]] && COVER_RATIO="$COVER_RATIO_ARG"`(flag 解析:`--cover-ratio` 的值存入 `COVER_RATIO_ARG`);`--cover-ratio` 指定其他值时先 `python3 ~/.claude/skills/smart-illustrator/scripts/minimax_t2i.py -h` 实测支持列表,不支持 → 警告"脚本不支持比例 ${X},已回退 16:9"
5. key/脚本检查(同 1.2 gemini 分支)→ 调用:

```bash
python3 ~/.claude/skills/smart-illustrator/scripts/minimax_t2i.py "$(cat /tmp/si-cover-prompt.txt)" \
...
3. engine 分支 —— **调用规范 + 格式统一与路径 1 完全相同**(gemini 走实测 minimax 契约;mermaid/excalidraw 走作者脚本):
   - gemini:HEREDOC 写 prompt 到 `/tmp/si-prompt-${NN}.txt`(≤1500 压缩策略同 Step 4a)→ `python3 ~/.claude/skills/smart-illustrator/scripts/minimax_t2i.py "$(cat ...)" --out /tmp/si-out-${NN} --ratio 16:9` → `mv /tmp/si-out-${NN}/minimax-0.jpeg {PNG}` → **格式统一规范(Step 4a 末尾):file 检测,JPEG 则 ffmpeg 转码为真 PNG**
...
- 默认 `16:9`(minimax 实测支持);`--cover-ratio <X:Y>` 指定其他比例(如 3:4 小红书)时,先 `python3 ~/.claude/skills/smart-illustrator/scripts/minimax_t2i.py -h` 实测支持列表,不支持 → 警告"脚本不支持比例 ${X},已回退 16:9"
...
| minimax 调用参数错误(如误用 --prompt-file/--output) | 立即 `python3 ~/.claude/skills/smart-illustrator/scripts/minimax_t2i.py -h` 实测契约修正后重试 |
